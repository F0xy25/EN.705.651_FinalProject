...
def initialize_system_state(state: State):
    ...
    for i in range(random.randrange(2, 5)):
        ...
        state['Users'][i]['User_id'] = i
        state['Users'][i]['current_sentiment'] = ''
        state['Users'][i]['guests_happy'] = False
        state['Users'][i]['group_preferences'] = GroupPreferences()
...
def call_node_2(state):
    ...
    for user in state['Users']:
        PROMPT = PromptTemplate(
            ...
            partial_variables={
                "current_sentiment": lambda: user.get('current_sentiment', ""),
            },
        ...
        user.update({"current_sentiment": parsed_output.current_sentiment})
...
def call_node_3(state):
    ...
    for user in state['Users']:
        ...
        prompt = PROMPT.format(
            current_sentiment=user["current_sentiment"])
        ...
        print("input state action prediction: ")
        print(state['building_event_state'])
        user.update({"guests_happy": parsed_output.guests_happy})

    # make decision to either aggregate guest happiness or check for key guest happiness:
    #aggregate_guest_happiness(state)
    check_key_guest_happiness(state)

    return state

def check_key_guest_happiness(state):
    state['group_preferences'] = state['Users'][0]['group_preferences']#promotes key user's preferences
    if state['Users'][0]['guests_happy'] == 'true':
...
def aggregate_guest_happiness(state):
    total_guests = len(state['Users'])
    THRESHHOLD = .4 #change threshold from 0-1.
    count = 0
    print(f"guests happiness:{[user['guests_happy'] for user in state['Users']]}")
    for user in state['Users']:
        if user['guests_happy'] == 'true':
            count = count + 1
...
workflow.add_conditional_edges(
    "Node 2: User Sentiment Simulation Node", is_event_done_yet_condition,
    {"Node 2: User Sentiment Simulation Node": "Node 3: Sentiment Analysis Node", "FINISH": "__end__"})
workflow.add_edge(
...
workflow.add_conditional_edges(
    ...
    {"Node 1: Changing Environment Simulation Node": "__end__",
     "Node 4: Environment Updater Node": "Node 4: Environment Updater Node"})
workflow.add_edge(
    "Node 4: Environment Updater Node", "Node 5: Tool Node")
workflow.add_conditional_edges(
    "Node 5: Tool Node", is_event_done_yet_condition,
    {"Node 2: User Sentiment Simulation Node": "Node 2: User Sentiment Simulation Node", "FINISH": "__end__"})
